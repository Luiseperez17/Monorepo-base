...
export class LoginComponent implements OnInit {
  ...
  //retenciones
  // verificadorRetenciones(){
  //   //verifico si hay conexion
  //   if (this.isOnline) {
  //     this.sUsuario.getRetenciones().subscribe((retenciones: any) => {
  //       if (retenciones.length != 0) {
  //         // Procedo a borrar la tabla de los retenciones
  //         this.loginDb.borrarRetenciones().then(() => {
  //           let contU = 0;
  //           //proceso de ingreso a la db
  //           for (let usuario of retenciones) {
  //             this.loginDb.addData(usuario, 'app_retenciones');
  //             contU++;
  //           }
  //         });
  //       } else {
  //         this.funciones.log("No hay retenciones para insertar");
  //       }
  //     },(error:any) => {
  //       console.error("Error al obtener retenciones:", error);
  //     });
  //   } else {
  //     this.funciones.log("No fue posible borrar los retenciones para luego cargarlos de nuevo. Motivo: No hay internet");
  //   }
  // }  
  ...
  async iniciarSesion(){
    if(this.login.Usuario == ""){
      ...
      const dbName = (environment.demo) ? environment.dbNameDemo : environment.dbName;
      const dbVersion = (environment.demo) ? environment.dbVersionDemo : environment.dbVersion;
